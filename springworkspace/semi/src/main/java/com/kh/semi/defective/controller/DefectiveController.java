...
public class DefectiveController {
    ...
    //상품 상세 조회
    @GetMapping("detail")
    public void detail(String bno, Model model){
        List<DefectiveVo> voList = service.getDefectiveDetail(bno, model);
    ...
    public String write(DefectiveVo vo, HttpSession session) throws Exception {
        ...
        if(result == 1){
            ...
            return "redirect:/qa/defective/write";
        }else{
    ...
    public String delete(String defectiveNoArr)throws JsonProcessingException {
        System.out.println("defectiveNoArr = " + defectiveNoArr);
